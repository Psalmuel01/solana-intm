...
describe("escrow-program-demo", async () => {
  ...
  const [programAuthority, programAuthBump] =
  ...
  it("Minting tokens", async () => {
    ...
    userATokenA = await createAssociatedTokenAccount(
      connection,
      userA,
      tokenA,
      userA.publicKey
    );
    userATokenB = await createAssociatedTokenAccount(
      connection,
      userA,
      tokenB,
      userA.publicKey
    );
    userBTokenA = await createAssociatedTokenAccount(
      connection,
      userB,
      tokenA,
      userB.publicKey
    );
    userBTokenB = await createAssociatedTokenAccount(
      connection,
      userB,
      tokenB,
      userB.publicKey
    ...
    // mint to users
    await mintTo(connection, userA, tokenA, userATokenA, payer, 100000000);

    await mintTo(connection, userB, tokenB, userBTokenB, payer, 100000000);
  });
  ...
  it("Initialize Escrow!", async () => {
    ...
    const tx = await program.methods
      .initializeEscrow(new BN(5), new BN(15))
      .accounts({
  ...
  it("Execute escrow swap!", async () => {
    ...
    await connection.confirmTransaction(tx);
    if (
      userATokenB === undefined ||
      userBTokenA === undefined ||
    ...
    let escrowStateAcct = await program.account.escrowState.fetch(escrowState);
    assert(
      Number(userATokenBAcct.amount) ==
        escrowStateAcct.requestedAmountB.toNumber()
    );
    assert(
      Number(userBTokenAAcct.amount) == escrowStateAcct.amountA.toNumber()
    );
  });
});
